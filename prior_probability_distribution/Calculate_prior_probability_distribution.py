...
from pipeline_custom.pipeline_UWLGM import UWLGMPipeline
from pipeline_custom.pipeline_stable_diffusion import StableDiffusionPipeline

from mmcv import Config
from mmdet.datasets import build_dataset
from utils.data.nuimage import NuImageDataset
from utils.data.ruod import RUODDataset
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default='/mnt/data0/mly/GeoModel/stable-diffusion-v1-5',
        required=True,
    ...
    parser.add_argument("--bucket_sincos_embed", action="store_true",
                        help="Whether to use 2D sine-cosine embedding for bucket locations")
    ...
    parser.add_argument("--foreground_loss_mode", type=str, default='constant', help="None, constant and area")

    parser.add_argument("--foreground_loss_weight", type=float, default=2.0,
                        help="Might be utilized differently with respect to loss mode")
    ...
    parser.add_argument("--feat_size", type=int, default=64,
                        help="Feature size of LDMs. Default to be 64 for stable diffusion v1.5")

    # unconditional generation specifics
    parser.add_argument("--uncond_prob", type=float, default=0.1,
                        help="Probability to downgrade to unconditional generation")

    # data specifics
    parser.add_argument(
        "--dataset_config_name",
        type=str,
        default='configs/data/coco_stuff_256x256.py',
    ...
    parser.add_argument(
        "--lr_warmup_steps", type=int, default=3000, help="Number of steps for the warmup in the lr scheduler."
    )
    parser.add_argument(
...
class SplitEmbedding(nn.Module):
    def __init__(self, embed, num_fixed):
        ...
        print("Spliting original embedding with shape ({}, {}) to ({}, {}) and ({}, {})".format(self.num_embeddings,
                                                                                                self.embedding_dim,
                                                                                                self.num_fixed,
                                                                                                self.embedding_dim,
                                                                                                self.num_tuned,
...
def main():
    ...
    if not args.train_text_encoder:
        ...
        if "added_embedding" in args.train_text_encoder_params:
            text_encoder.text_model.embeddings.token_embedding = SplitEmbedding(
                text_encoder.text_model.embeddings.token_embedding, num_words)
    ...
    def collate_fn(examples):
        ...
        padded_tokens = tokenizer.pad({"input_ids": input_ids}, padding=True,
                                      return_tensors="pt")  # pad to the longest of current batch (might differ between cards)
        bbox_mask = torch.stack([example["bbox_mask"] for example in examples]).unsqueeze(1).to(
            memory_format=torch.contiguous_format).float() if args.foreground_loss_mode else None  # [B, 1, H, W]
        cls_mask = [example["cls_mask"] for example in examples]
        ori_filename = [example["ori_filename"] for example in examples]
        return {
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            print(f'正在处理第{epoch+1}个批次，第{step+1}个步骤')
            ...
            for i in range(latents.size(0)):
                # 按照类别分割图像区域
                for key, value in batch['cls_mask'][i].items():
                    if key in save_dict:
                        pass
                    else:
                        save_dict[key] = []
                    # 在选定图像、选定类别的基础上，按照不同通道进行区域数据的提取
                    for num_ch in range(latents[i].shape[0]):
                        # 取出当前通道的数据
                        channel_b = latents[i][num_ch, :, :]
                        # 逐元素比较A和channel_b（但只关心A中非零的部分）
                        mask = value != 0  # 创建一个与A形状相同的布尔型张量，A中非零位置为True
                        # 使用mask来索引channel_b，得到符合条件的元素
                        selected_elements = channel_b[mask]
                        # 将这些元素添加到对应通道的列表中
                        save_dict[key].append(selected_elements.tolist())  # 转换为列表并添加

    with open('/home/mly/UWLGM/prior_probability_distribution/data.json', 'w') as f:
