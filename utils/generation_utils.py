...
import torch
import diffusers
from pipeline_custom.pipeline_prior import PriorDiffusionPipeline
from pipeline_custom.pipeline_noisecollege import NoiseCollegePipeline
from pipeline_custom.pipeline_priornoisefusion import PriorNoiseFusionPipeline
...
dataset2classes = dict(
    ...
    coco_stuff=dict(
        classes=[
            ...
            'desk-stuff', 'dirt',
            'door-stuff', 'fence', 'floor-marble', 'floor-other', 'floor-stone', 'floor-tile', 'floor-wood', 'flower',
            'fog', 'food-other',
    ...
    ruod=dict(
        classes=[
            "holothurian", "echinus", "scallop", "starfish", "fish", "corals", "diver", "cuttlefish", "turtle",
            "jellyfish"
        ]
    ),
    urpc=dict(
...
dataset2classes['coco_stuff']['class2text'] = {each: each.replace('-', ' ') for each in
                                               dataset2classes['coco_stuff']['classes']}
dataset2classes['ruod']['class2text'] = {each: each for each in dataset2classes['ruod']['classes']}
dataset2classes['urpc']['class2text'] = {each: each for each in dataset2classes['urpc']['classes']}
...
def load_checkpoint(ckpt_path, pipeline=PriorDiffusionPipeline):
...
def bbox_encode(bboxs, configs):
    ...
    assert len(bboxs) <= configs["max_num_bbox"], 'number of bboxes should not exceed {}, but got {}'.format(
        configs["max_num_bbox"], len(bboxs))

    objs = []
    for bbox in bboxs:
        # bbox sanity check
        label, bbox = bbox[0], bbox[1:]
        assert label in configs["dataset2classes"][
            "classes"], "bbox class should be selected from the {} dataset, but got {}".format(configs['dataset'],
                                                                                               label)
        assert bbox[2] > bbox[0] and bbox[3] > bbox[1] and bbox[0] >= 0 and bbox[1] >= 0 and bbox[2] <= 1 and bbox[
            3] <= 1, "bbox coord should be in the (x1, x2, y1, y2) format, but got {}".format(bbox)

        # bbox encode
        bbox = tokenize_coordinates(bbox[0], bbox[1], configs["num_bucket_per_side"]) + ' ' + tokenize_coordinates(
            bbox[2], bbox[3], configs["num_bucket_per_side"])
...
def input_ids2caption_column(input_ids, attention_mask):
