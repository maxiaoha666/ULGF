    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if self.prompt_version == 'v1':
                ...
                location_list = []
                weight_dict = {}
                # num在循环中舍弃一些框（开始、结束在一个点），所有要有一个记录目前损失数量的
                num_dui = 0
                ...
                def deal_with_mask(targets,weight_dict,ori_filename):
                    # 存储点对应的目标
                    point_to_targets = {}
                    ...
                    grouped_by_value_result = {key: 99 for key in grouped_by_value}
                    ...
                    for num, (target_list, points) in enumerate(grouped_by_value.items()):
                        ...
                        if len(target_list) == 3:
                            try:
                                count_of_keys_with_value_zero = weight_dict[target_list[0]]
                                count_of_keys_with_value_one = weight_dict[target_list[1]]
                                count_of_keys_with_value_two = weight_dict[target_list[2]]
                            except KeyError:
                                print(1)
                            count_of_keys_with_value_zot = sorted([count_of_keys_with_value_zero,count_of_keys_with_value_one,count_of_keys_with_value_two],key=lambda x: x.item())
                            r = random.random()
                            if r < 0.31:
                                grouped_by_value_result[target_list] = next((key for key, value in weight_dict.items() if value == count_of_keys_with_value_zot[0]), None)
                            elif r < 0.64:
                                grouped_by_value_result[target_list] = next((key for key, value in weight_dict.items() if value == count_of_keys_with_value_zot[1]), None)
                            else:
                                grouped_by_value_result[target_list] = next((key for key, value in weight_dict.items() if value == count_of_keys_with_value_zot[2]), None)
                            # 取代原来的完全随机
                ...
                final_point_to_target = deal_with_mask(targets=location_list,weight_dict=weight_dict,ori_filename = ori_filename)
