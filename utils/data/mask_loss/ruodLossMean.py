    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if self.prompt_version == 'v1':
                ...
                # 记录被除数
                bbox_mask_cnt = torch.zeros(self.FEAT_SIZE).float()
                for each in index:
                    label = labels[each]
                    bbox = bboxes[each]
                    ...
                    if label in ['truck', 'bicycle', 'motorcycle']:
                        ...
                        if self.foreground_loss_mode == 'constant':
                            ...
                            bbox_mask_batch = torch.zeros(self.FEAT_SIZE).float()
                            bbox_mask_batch[coord[1]: coord[3], coord[0]: coord[2]] = self.foreground_loss_weight * 1 / torch.pow(torch.tensor((coord[3] - coord[1] + 1) * (coord[2] - coord[0] + 1)), 0.2)
                            bbox_mask_cnt_batch = torch.zeros(self.FEAT_SIZE).float()
                            bbox_mask_cnt_batch[coord[1]: coord[3], coord[0]: coord[2]] = 1
                            bbox_mask = bbox_mask_batch + bbox_mask
                            bbox_mask_cnt = bbox_mask_cnt_batch + bbox_mask_cnt
                            '''
                ...
                tensor_dict = {
                    ori_filename: bbox_mask_cnt,  # 示例 Tensor
                }
                bbox_mask = bbox_mask_cnt
                '''
                # torch.save(tensor_dict, '/mnt/data0/mly/RUOD/每张图像Loss掩码输入文件/' + ori_filename + '.pth')
                bbox_mask_cnt[bbox_mask_cnt == 0] = 1
                bbox_mask = bbox_mask / bbox_mask_cnt
