                ...
                for each in index:
                    ...
                    if label in ['truck', 'bicycle', 'motorcycle']:
                        ...
                        if self.foreground_loss_mode == 'constant':
                            region = bbox_mask[coord[1]: coord[3], coord[0]: coord[2]]
                            region_value = self.foreground_loss_weight * 1 / torch.pow(torch.tensor((coord[3] - coord[1] + 1) * (coord[2] - coord[0] + 1)), 0.2)
                            '''为像素权重损失最大化
                            region[region <= region_value] = region_value'''
                            '''为像素权重损失最小化
                            region[(region >= region_value) | (region < 0.0001)] = region_value'''
                            '''为像素权重随机化'''
                            '''
                            Random_hyperparameters = random.random()
                            if Random_hyperparameters < 0.5:
                                region[region <= region_value] = region_value
                            else:
                                region[(region >= region_value) | (region < 0.0001)] = region_value
                            bbox_mask[coord[1]: coord[3], coord[0]: coord[2]] = region
                            '''
