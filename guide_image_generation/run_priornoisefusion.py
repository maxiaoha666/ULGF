...
def run_layout_to_image(layout, save_label, pipe, generation_config, denoise_img):
    ...
    # 导入先验信息文件
    with open('/home/mly/UWLGM/prior_probability_distribution/prior_distribution.json', 'r') as f:
    ...
    images = pipe.__call__(n_samples * prompt_list, guidance_scale=scale, num_inference_steps=num_inference_steps,
                           height=int(height), width=int(width), masks=masks, latents=prior_tensor.unsqueeze(0)).images
    ########################
    # Save results
    #########################
